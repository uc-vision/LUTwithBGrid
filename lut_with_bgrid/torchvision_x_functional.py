...
def adjust_brightness(img, value=0):
    ...
    result = np.clip(img.astype(np.float32)+value, dtype_min, dtype_max).astype(dtype)
...
def adjust_contrast(img, factor):
    ...
    result = np.clip(img.astype(np.float32)*factor, dtype_min, dtype_max).astype(dtype)
